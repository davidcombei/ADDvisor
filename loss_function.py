...
class LMACLoss():
    ...
    def loss_function(self, xhat, X_stft_power,X_stft_phase, class_pred):
        ...
        relevant_mask_feats = audio_processor.extract_features_istft(istft_relevant_mask)
        irelevant_mask_feats = audio_processor.extract_features_istft(istft_irelevant_mask)
        relevant_mask_feats = torch.mean(relevant_mask_feats.squeeze(0), dim=0)
        irelevant_mask_feats = torch.mean(irelevant_mask_feats.squeeze(0), dim=0)
